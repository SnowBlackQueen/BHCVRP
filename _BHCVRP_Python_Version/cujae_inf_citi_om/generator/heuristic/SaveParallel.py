...
class SaveParallel(Save):
    ...
    def get_solution_inicial(self):
        ...
        if Problem.get_problem().get_type_problem() == ProblemType.CVRP or Problem.get_problem().get_type_problem() == ProblemType.HFVRP or Problem.get_problem().get_type_problem() == ProblemType.OVRP or Problem.get_problem().get_type_problem() == ProblemType.TTRP:
            ...
            customers_to_visit = Problem.get_problem().get_list_customers()
        else:
            i = 0
            found = False
            
            depot = Problem.get_problem().get_list_depots().get(i)

            while i < len(Problem.get_problem().get_list_depots()) and not found:
                if depot.get_list_assigned_customers():
                    pos_depot = i
        ...
        list_capacities = list(Problem.get_problem().fill_list_capacities(pos_depot))  
        ...
        row_col = np.unravel_index(np.argmax(save_matrix), save_matrix.shape)
        route_row = Route()
        route_col = Route()
