...
class SaveParallel(Save):
    ...
    def initialize_specifics(self):
        ...
        self.row, self.col = np.unravel_index(
            np.argmax(self.save_matrix), self.save_matrix.shape
        )
    ...
    def creating(
        ...
        if (
            ...
            if self.compatible_routes(
                self.route_row, self.route_col
            ) or self.compatible_routes(self.route_col, self.route_row):
    ...
    def processing(
        ...
        if (
            ...
            if self.save_value > 0 or (
                ...
                if (
                    self.save_value <= 0
                    and len(self.list_routes) <= self.count_vehicles
                ):
            ...
            if self.counter == self.iterations or np.all(self.save_matrix == -np.inf):
                ...
                self.is_open = True
                self.update_customers_to_visit(
                    self.close_route, self.customers_to_visit
                )
    ...
    def execute(self):
        if self.type_problem == 0 or self.type_problem == ProblemType.CVRP or self.type_problem == ProblemType.OVRP or self.type_problem == 5 or self.type_problem == ProblemType.SBRP or self.type_problem == 6 or self.type_problem == ProblemType.VRPTW:
            while (
                ...
                and not np.all(self.save_matrix == -np.inf)
            ):
            ...
            while self.list_routes and self.list_capacities:
                if (self.counter == self.iterations) and (
                    not self.is_open
                ):  # es q recorri ya la matriz completa
                    self.close_route = self.route_to_close(self.list_routes)
                    self.close_route.set_id_depot(self.id_depot)
                ...
                if not self.is_first:
                    ...
                    self.iterations = (
                        self.cant_customers * (self.cant_customers - 1)
                    ) / 2
                    self.counter = 0
            ...
            if len(self.list_routes) > 0:
                ...
                while self.list_routes:
                    ...
                    self.route.get_list_id_customers().extend(
                        self.list_routes[0].get_list_id_customers()
                    )
                    self.list_routes.pop(0)
            ...
            for j in range(
                ...
                if j != self.pos_depot:
                    ...
                    self.list_capacities = list(
                        Problem.get_problem().fill_list_capacities(self.pos_depot)
                    )  # fill o listparametro con el depot
    ...
    def inspect_routes(
        self, list_routes, list_capacities, solution, customers_to_visit
    ):
        for i in range(len(list_routes)):
            ...
            while (
                j < len(list_capacities) and not found
            ):  # se debe eliminar la capacidad de la lista
    ...
    def update_customers_to_visit(self, close_route, customers_to_visit):
        for i in range(len(close_route.get_list_id_customers())):
            ...
            while j < len(customers_to_visit) and not found:
                if (
                    customers_to_visit[j].get_id_customer()
                    == close_route.get_list_id_customers()[i]
                ):
    ...
    def compatible_routes(self, route_ini, route_end):
        ...
        if (
            isinstance(route_ini, RouteTTRP)
            and route_ini._type_route == RouteType.PTR
            and not isinstance(route_end, RouteTTRP)
        ):
            is_compatible = False
