...
class MatchingBasedSavingAlgorithm(Save):
    ...
    def initialize_specifics(self):
        ...
        if self.type_problem == 2 or self.type_problem == ProblemType.MDVRP:
            self.customers = Problem.get_problem().get_customers_assigned_by_id_depot(
    ...
    def get_solution_inicial(self):
        if self.type_problem == 0 or self.type_problem == ProblemType.CVRP or self.type_problem == 5 or self.type_problem == ProblemType.SBRP or self.type_problem == ProblemType.VRPTW or self.type_problem == 6:
            ...
            self.type_problem == 2 or self.type_problem == ProblemType.MDVRP
        ):  # REVISAR
            self.repeat = False
                ...
                if j != self.pos_depot:
                    ...
                    if self.customers:
                        ...
                        self.save_matrix = self.fill_save_matrix(
                            self.id_depot, self.customers
                        )

                while (
                    self.list_routes
                    and not self.repeat
                    and not self.depot_finish
                    and self.customers
                ):
                    self.list_routes = self.match_based_savings_algorithm(
                        self.customers, self.depots
            ...
            for j in range(i + 1, len(elements)):
                ...
                if self.type_problem == 2 or self.type_problem == ProblemType.MDVRP:
                    ci = Problem.get_problem().get_pos_element_by_id_depot(
                        self.id_depot, c_i.get_id_customer(), self.depots
                    )
                ...
                if combined_demand <= fleet_capacity:
                    ...
                    if self.type_problem == 1 or self.type_problem == ProblemType.HFVRP:
                        ...
                        self.update_customers_to_visit(
                            merged_route, self.customers_to_visit
                        )
            ...
            if self.type_problem == 2 or self.type_problem == ProblemType.MDVRP:
                for c in self.customers:
                    for r in range(len(self.list_routes)):
                        for i in range(
                            len(self.list_routes[r].get_list_id_customers())
                        ):
                            if (
                                c.get_id_customer()
                                == self.list_routes[r].get_list_id_customers()[i]
                            ):
