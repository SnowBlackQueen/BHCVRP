...
class RouteTTRP(Route):
    ...
    def __init__(
        ...
        if (
            ...
            and list_access_vc is not None
            and type_route is not None
        ):
            super().__init__(
                list_id_customers,
                request_route,
    ...
    def get_cost_route_with_sub_tour(self, list_id_customers: List[int]) -> float:
        ...
        for i in range(1, len(list_id_customers)):
            ...
            type_customer_next = Problem.get_problem().get_type_by_id_customer(
                customer_next
            )

            if (
                type_customer_ini == CustomerType.VC
                and type_customer_next == CustomerType.VC
                and not vc_in_sub
            ):
                ...
                if (
                    type_customer_ini == CustomerType.VC
                    and type_customer_next == CustomerType.TC
                    and not vc_in_sub
                ):
                    ...
                    # cost_route += Problem.getProblem().getCostMatrix().getItem(pos_customer_ini, pos_customer_next)
                    request_sub_route += (
                        Problem.get_problem().get_request_by_id_customer(customer_next)
                    )
                else:
                    if (
                        type_customer_ini == CustomerType.TC
                        and type_customer_next == CustomerType.TC
                    ) or (
                        type_customer_ini == CustomerType.VC
                        and type_customer_next == CustomerType.TC
                        ...
                        if request_sub_route <= capacity_vehicle:
                            ...
                            cost_route += (
                                Problem.getProblem()
                                .getCostMatrix()
                                .getItem(pos_customer_ini, pos_customer_next)
                            )
                        else:
                            request_sub_route = (
                                Problem.get_problem().get_request_by_id_customer(
